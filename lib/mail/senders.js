...
class PaiementConfirmeEmail extends EmailBase {
  ...
  sujet() {
    if (this.billets.length > 1) {
      return `Tes ${this.billets.length} billets ${this.eventName}`.trim();
    }
  ...
  contenu() {
    ...
    if (groupe) {
      return {
        titre: `Tes ${this.billets.length} billets sont confirmés`,
        intro: prenom ? `Bonjour ${prenom}, tes places sont réservées.` : "Tes places sont réservées.",
        paragraphes: [
          "Chaque personne a son propre code et son propre billet. Remets à chacun le sien : "
          + "un code ne sert qu'une fois, et l'entrée se fait au nom inscrit dessus.",
          "Tous les billets sont joints à ce message en PDF.",
        ],
        ...
        boutonTexte: "Retrouver mes billets",
        boutonLien: this.absolu("/#verifier"),
        piedNote:
          "Chaque billet est personnel et ne sert qu'une fois. Tu peux les retrouver à tout moment "
    ...
    return {
      ...
      intro: prenom ? `Bonjour ${prenom}, ta place est réservée.` : "Ta place est réservée.",
      paragraphes: [
      ...
      piedNote:
        "Ce billet est personnel et ne sert qu'une fois. Le billet PDF est joint à ce message.",
    };
  }
}
...
class CodeVerificationEmail extends EmailBase {
  ...
  sujet() {
    return `${this.code} — ton code pour récupérer ton billet ${this.eventName}`;
  }

  contenu() {
    return {
      titre: "Ton code de vérification",
      intro: "Saisis-le sur le site pour retrouver ton billet.",
      paragraphes: [
      ...
      piedNote:
        "Si tu n'as pas demandé ce code, ignore ce message : personne ne peut accéder à ton billet sans lui.",
    };
  }
}
